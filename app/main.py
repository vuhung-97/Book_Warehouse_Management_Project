...
@api.post("/book_types", response_model=BookType, status_code=201, summary="Thêm loại sách mới")
def create_book_type(book_type: BookTypeBase):
    try:
...
def get_book_type_by_id(book_type_id: int):
    try:
        book_type = crud.get_book_type_by_id_db(book_type_id)
        if not book_type:
...
def delete_book_type(book_type_id: int):
    try:
        deleted = crud.delete_book_type_db(book_type_id)
        if not deleted:
            raise HTTPException(status_code=404, detail="Không tìm thấy loại sách")
...
@api.post("/publishers", response_model=Publisher, status_code=201, summary="Thêm nhà xuất bản mới")
def create_publisher(publisher: PublisherBase):
    try:
...
def get_publisher_by_id(publisher_id: int):
    try:
        publisher = crud.get_publisher_by_id_db(publisher_id)
        if not publisher:
...
def update_publisher(publisher_id: int, publisher: PublisherBase):
    try:
        updated_publisher = crud.update_publisher_db(publisher_id, publisher)
...
def delete_publisher(publisher_id: int):
    try:
        deleted = crud.delete_publisher_db(publisher_id)
        if not deleted:
            raise HTTPException(status_code=404, detail="Không tìm thấy nhà xuất bản")
...
@api.post("/books", response_model=Book, status_code=201, summary="Thêm sách mới")
...
def get_all_books():
    try:
        return crud.get_all_books_with_names_db()
...
def get_book_by_id(book_id: int):
    try:
        book = crud.get_book_by_id_with_names_db(book_id)
        if not book:
...
@api.put("/books/{book_id}", response_model=Book, summary="Cập nhật sách")
def update_book(book_id: int, updated_book: BookBase):
    try:
        book = crud.update_book_db(book_id, updated_book)
        if not book:
            raise HTTPException(status_code=404, detail="Không tìm thấy sách")
        return book
    except HTTPException as e:
...
def delete_book(book_id: int):
    try:
        deleted = crud.delete_book_db(book_id)
        if not deleted:
            raise HTTPException(status_code=404, detail="Không tìm thấy sách")
...
def get_books_by_name(book_name: str):
    try:
        books = crud.get_books_by_name_db(book_name)
        if not books:
...
@api.get("/books/search/author", response_model=List[BookWithNames], summary="Tìm sách theo tên tác giả")
def get_books_by_author(author: str):
    try:
        books = crud.get_books_by_author_db(author)
        if not books:
...
@api.get("/books/search/publisher", response_model=List[BookWithNames], summary="Tìm sách theo tên nhà xuất bản")
def get_books_by_publisher_name(publisher_name: str):
    try:
        books = crud.get_books_by_publisher_name_db(publisher_name)
        if not books:
...
@api.get("/books/search/type", response_model=List[BookWithNames], summary="Tìm sách theo tên loại sách")
def get_books_by_type_name(book_type_name: str):
    try:
        books = crud.get_books_by_type_name_db(book_type_name)
        if not books:
...
@api.get("/books/search/publisher_id", response_model=List[BookWithNames], summary="Tìm sách theo ID nhà xuất bản")
def get_books_by_publisher_id(publisher_id: int):
    try:
        books = crud.get_books_by_publisher_id_db(publisher_id)
        if not books:
            raise HTTPException(status_code=404, detail="Không tìm thấy sách nào cho nhà xuất bản này")
        return books
    except HTTPException as e:
...
@api.get("/books/search/type_id", response_model=List[BookWithNames], summary="Tìm sách theo ID loại sách")
def get_books_by_type_id(type_id: int):
    try:
        books = crud.get_books_by_type_id_db(type_id)
        if not books:
            raise HTTPException(status_code=404, detail="Không tìm thấy sách nào cho loại sách này")
        return books
    except HTTPException as e:
